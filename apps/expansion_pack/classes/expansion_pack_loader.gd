...
func load_packs(verbose: bool = true) -> Array[ExpansionPackResource]:
	if verbose: print_rich("[color=white][b][ExpansionPackLoader][/b][/color]:")
	...
	if verbose: print(" • Discovered PCK files: ", discovered_pck_files)
	...
	elif verbose: print(" • Can't unpackage PCK files in editor build, skipping")
	...
	if verbose: print(" • Discovered resource files: ", discovered_resource_files)
	...
	if verbose: print_rich("[color=white]Finished loading expansion packs![/color]\n")
